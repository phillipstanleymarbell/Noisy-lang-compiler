...
SourceInfo *	noisyLexAllocateSourceInfo(	State *  N, char **  genealogy, 
							char *  fileName, uint64_t lineNumber,
							uint64_t columnNumber, uint64_t length);
Token *		noisyLexAllocateToken(		State *  N, IrNodeType type, 
							char *  identifier, uint64_t integerConst,
							double realConst, char * stringConst,
							SourceInfo *  sourceInfo);
void			noisyLexPut(State *  N, Token *  newToken);
Token *		noisyLexGet(State *  N, const char *tokenDescriptionArray[kNoisyIrNodeTypeMax]);
Token *		noisyLexPeek(State *  N, int lookAhead);

void			noisyLexPrintToken(State *  N, Token *  t, const char *tokenDescriptionArray[kNoisyIrNodeTypeMax]);
void			noisyLexDebugPrintToken(State *  N, Token *  t, const char *tokenDescriptionArray[kNoisyIrNodeTypeMax]);
void			noisyLexPeekPrint(State *  N, int maxTokens, int formatCharacters, const char *tokenDescriptionArray[kNoisyIrNodeTypeMax]);
